                     ...
                     [(?bool? e)
                      ...
                      [(valof? e)
                       (letrec ([inic (lambda (s env)
                         ...
                         (inic (valof-s e) env))]
                      ; Funkcija
                      [(fun? e) ; Neuzitna koda incoming
                       (cond
                         ;[(not (list? (fun-farg e))) (closure env e)] ; to list
                         [(not (equal? (fun-farg e) (remove-duplicates (fun-farg e)))) (friq (trigger (exception "fun: duplicate argument identifier")) env)]
                         [#t
                          (letrec ([remove_env_var (lambda (env e) ; remove_env_var -> get body var
                                           (cond
                                             [(vars? e)
                                              (letrec ([find_arg (lambda (env arg)
                                                                   (cond
                                                                     [(null? env) '()]
                                                                     [(equal? (car (car env)) arg) (append (list (car env)) (find_arg (cdr env) arg))]
                                                                     [#t (find_arg (cdr env) arg)]))])

                                                (append (find_arg env (vars-s e)) (remove_env_var env (vars-e1 e)) (remove_env_var env (vars-e2 e))))]
                                             [(..? e) (append (remove_env_var env (..-e1 e)) (remove_env_var env (..-e2 e)))]
                                             [(handle? e) (append (remove_env_var env (handle-e1 e)) (remove_env_var env (handle-e2 e)) (remove_env_var env (handle-e3 e)))]
                                             [(if-then-else? e) (append (remove_env_var env (if-then-else-condition e)) (remove_env_var env (if-then-else-e1 e)) (remove_env_var env (if-then-else-e2 e)))]
                                             [(?int? e) (append (remove_env_var env (?int-e e)))]
                                             [(?bool? e) (append (remove_env_var env (?bool-e e)))]
                                             [(?..? e) (append (remove_env_var env (?..-e e)))]
                                             [(?seq? e) (append (remove_env_var env ?seq-e e))]
                                             [(?empty? e) (append (remove_env_var env (?empty-e e)))]
                                             [(?exception? e) (append (remove_env_var env (?exception-e e)))]
                                             [(add? e) (append (remove_env_var env (add-e1 e)) (remove_env_var env (add-e2 e)))]
                                             [(mul? e) (append (remove_env_var env (mul-e1 e)) (remove_env_var env (mul-e2 e)))]
                                             [(?leq? e) (append (remove_env_var env (?leq-e1 e)) (remove_env_var env (?leq-e2 e)))]
                                             [(?=? e) (append (remove_env_var env (?=-e1 e)) (remove_env_var env (?=-e2 e)))]
                                             [(head? e) (append (remove_env_var env (head-e e)))]
                                             [(tail? e) (append (remove_env_var env (tail-e e)))]
                                             [(~? e) (append (remove_env_var env (~-e e)))]
                                             [(valof? e) (list (cons (valof-s e) #t))] ; <----
                                             [(proc? e) (append (remove_env_var env (proc-body e)))]
                                             [(fun? e) (append(remove_env_var env (fun-body e)) (list (fun-name e)))] ; function arguments are not important?
                                             [#t '()]))])
                            (let ([v1 (append (remove_env_var env e) (fun-farg e))])
                              (letrec ([clear_vars (lambda (env)
                                                     (cond
                                                       [(null? env) '()]
                                                       ;[#t (if (or (member (car env) v1) (member (car (car env)) v1)) (clear_vars (cdr env)) (append (list (car env)) (clear_vars (cdr env))))]
                                                       [#t (if (or (member (car env) v1) (member (car (car env)) v1)) (clear_vars (cdr env))
                                                               (if (member (cons (car (car env)) #t) v1) (append (list (car env)) (clear_vars (cdr env))) (clear_vars (cdr env))))]
                                                       ))])
                                (let ([v2 (clear_vars env)])
                                  (letrec ([remove_rec (lambda (z s)
                                                         (cond
                                                           [(null? z) '()]
                                                           [#t (if (member (car (car z)) s) (remove_rec (cdr z) s) (append (list (car z)) (remove_rec (cdr z) (append s (list (car (car z)))))))]))])
                                    (closure (remove_rec v2 (list)) e))))))])]
                                        
                      ;(closure (remove_env_var env e) e)
                      ; Skripta (procedure)
                      [(proc? e) e]
                      ; Klicanje
                      [(call? e)
...
(define-syntax folding
  (syntax-rules ()
    [(folding f init seq)
     (letrec ([fold_it (lambda (s v)
                         (cond
                           [(empty? s) v]
                           [(empty? (..-e2 s)) (fri (call f (list (..-e1 s) v)) null)]
                           [#t (fold_it (..-e2 s) (fri (call f (list (..-e1 s) v)) null))]))])
       (fold_it seq init))]))
